...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/editclass.css') }}">
</head>
<body>
    @include('layouts/app')
    <h1>Editar Aprendiz</h1>
    <form action="{{ route('aprendices.update', ['codigoA' => $codigoA]) }}" method="POST" enctype="multipart/form-data">

        @csrf
        @method('PUT')
        <input type="hidden" id="codigoA" name="codigoA" required value="{{ $codigoA }}">
        ...
        @if ($aprendiz->Imagen)
        <img src="{{ asset('storage/fotos_aprendiz/' . $aprendiz->Imagen) }}" alt="Foto de perfil">
        @endif
        <br>
